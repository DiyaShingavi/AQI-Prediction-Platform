...
ML_FEATURES = [
    ...
    "PM25_roll3","PM10_roll3","AQI_lag1","AQI_roll3",
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    <b>Input features:</b> 15 (including City_encoded)
    </div>
    """, unsafe_allow_html=True)
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    {"Model": "Linear Regression", "R² Score": 0.9759, "MAE": 7.95,  "RMSE": 12.11},
    {"Model": "Random Forest",     "R² Score": 0.9716, "MAE": 8.93,  "RMSE": 13.15},
    {"Model": "Extra Trees",       "R² Score": 0.9693, "MAE": 9.24,  "RMSE": 13.66},
    {"Model": "Gradient Boosting", "R² Score": 0.9676, "MAE": 9.49,  "RMSE": 14.05},
    {"Model": "SVR",               "R² Score": 0.9597, "MAE": 10.81, "RMSE": 15.67},
    {"Model": "XGBoost",           "R² Score": 0.9454, "MAE": 12.74, "RMSE": 18.24},
    {"Model": "LSTM",              "R² Score": 0.9236, "MAE": 15.25, "RMSE": 21.58},
    ])
    ...
    st.markdown(f"""
    <div class="card">
    <b style='color:#00d4aa;'>Research Insight: Multi-City Model Analysis</b><br><br>
    The multi-city model was trained using AQI data from six Indian cities: Mumbai, Delhi, Bengaluru,
    Hyderabad, Chennai, and Kolkata. This gives the model exposure to different pollution patterns,
    climate conditions, traffic density, and urban environments.<br><br>

    In our comparison, <b style='color:#ffd93d;'>Linear Regression achieved the highest R² score</b>.
    This suggests that, across these six cities, the relationship between major pollutants and AQI is
    largely linear and strongly dependent on pollutant concentration values.<br><br>

    This is an important finding because it shows that deep learning is not always superior. For this
    dataset, simpler interpretable models can perform better than complex models, making the system
    more efficient and easier to explain.
    </div>
    """, unsafe_allow_html=True)
    ...
    model_scores = {
        "Random Forest":     [0.97, 0.85, 0.80, 0.75, 0.30],
        "Extra Trees":       [0.97, 0.90, 0.75, 0.75, 0.30],
        "Gradient Boosting": [0.97, 0.70, 0.70, 0.70, 0.30],
        "XGBoost":           [0.95, 0.80, 0.65, 0.80, 0.35],
        "SVR":               [0.96, 0.60, 0.55, 0.60, 0.25],
        "Linear Regression": [0.98, 0.95, 0.95, 0.90, 0.20],
        "LSTM":              [0.92, 0.50, 0.40, 0.90, 0.95],
    }
    N = len(categories)
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    if st.button("▶ Start Simulation"):
        ...
        st.info("""
        This dashboard demonstrates a complete AQI prediction pipeline using multi-city AQI data,
        feature engineering, ML/DL model comparison, city-wise visualization, and simulated real-time prediction.
        """)
